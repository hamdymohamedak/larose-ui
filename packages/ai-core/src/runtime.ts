...
export interface AIRuntimeConfig {
  adapter: AIAdapter;
  /** Static list or live getter (Vue/Svelte stores). */
  grantedPermissions?: string[] | (() => string[]);
...
export function createAIRuntime(config: AIRuntimeConfig): AIRuntime {
  const getGranted = () =>
    typeof config.grantedPermissions === 'function'
  ...
  return {
    ...
    async filterTable(query, data, columns, permission) {
      ...
      const access = checkActionPermission('table.filter', getGranted(), permission);
    ...
    async populateForm(query, fields, permission) {
      const intent = parseIntent(query);
      const access = checkActionPermission('form.populate', getGranted(), permission);
