...
When a selected element has a React fiber attached, the inspector resolves the nearest component `displayName` and sanitized props (functions redacted, values truncated). Performance metrics come from `@larose-ui/observability-react` `performance` events when `ObservedComponent` wraps the target.
